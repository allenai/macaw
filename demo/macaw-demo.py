...
LOG_FILE = "macaw-demo.log"   # Where to save a log
...
def load_model_parallel(model_name_or_path, cuda_devices):
    ...
    print(f"{datetime.now()} Moving to GPU...")
...
def run_model(input_string, **generator_options):
    import torch
    with torch.no_grad():
        input_string = input_string  #  + " </s>" #  Add </s> if using Transformers < 3.1.0
        input_ids = st.TOKENIZER.encode(input_string, return_tensors="pt").to(st.CUDA_DEVICE)
        res = st.MODEL.generate(input_ids, **generator_options)
        output_strings = st.TOKENIZER.batch_decode(res, skip_special_tokens=True)
...
def run_model_with_outputs(input_string, output_strings):
    import torch
    with torch.no_grad():
        input_string = input_string  #  + " </s>" #  Add </s> if using Transformers < 3.1.0
        input_ids = st.TOKENIZER.encode(input_string, return_tensors="pt").to(st.CUDA_DEVICE)
        ...
        for output_string, output_text in output_strings:
            output_ids = st.TOKENIZER.encode(output_string, return_tensors="pt").to(st.CUDA_DEVICE)
            res = st.MODEL(input_ids, labels=output_ids, return_dict=True)
            ...
            all_res.append({
                ...
                "output_tokens": st.TOKENIZER.convert_ids_to_tokens(output_ids[0])
            })
    return all_res
...
if not hasattr(st, 'MODEL'):
    if MODEL_NAME_OR_PATH is None:
        ...
        st.MODEL, st.TOKENIZER, st.CUDA_DEVICE = load_model_parallel(MODEL_NAME_OR_PATH, CUDA_DEVICES)
...
SLOT_SHORTFORMS = {"Q": "Question", "C": "Context", "A": "Answer", "E": "Explanation",
                   "M": "MCOptions"}
ALL_SLOTS = ["Context", "Question", "MCOptions", "Answer", "Explanation"]
...
def make_input_from_example(example):
    ...
    for slot in ALL_SLOTS:
        if slot.lower() in slots:
            value = slots[slot.lower()]
            if value.strip() == "":
                res_out.append(slot[0])
            else:
                res_in.append(f"{slot[0]}: {value}")
    return "\n".join(res_in + res_out)
...
def render_output_slots(state, index):
    ...
    str_out = ""
    for slot in ALL_SLOTS:
        if slot.lower() in output_slots:
            str_out += "**" + slot + ":** " + output_slots[slot.lower()] + "  \n"
    #st.write("**" + slot + ":**", output_slots[slot.lower()])
    st.write(str_out)
...
EXAMPLES = [
    ...
    {"type":"DA-NoIR","id":"ARCEZ_MEAP_2005_8_43","angle":"EQ->A","input":"$answer$ ; $question$ = Astronauts weigh more on Earth than they do on the moon because ; $explanation$ = The earth has more mass than the moon. As the force of gravity increases, the weight of objects will increase. As mass of a celestial body increases, the force of gravity on that planet will increase.","output":"$answer$ = the moon has lower gravity"},
    {"type":"DA-NoIR","id":"ARCEZ_Mercury_7040863","angle":"Q->AE","input":"$explanation$ ; $answer$ ; $question$ = What system helps the body defend itself against disease while maintaining the levels of body fluids?","output":"$explanation$ = N\/A ; $answer$ = lymphatic"},
...
def main_page(state):
    st.title("Macaw demo")
    st.write("Macaw, Multi-Angle c(q)uestion answering model, is described [here](https://github.com/allenai/macaw)")
...
def compute_answer(raw_input, generator_options, random_tickle=1, output_strings=None):
    ...
    if MODEL_NAME_OR_PATH is not None:
        res = run_model(raw_input, **return_probs, **generator_options)
    else:
        res = {"input_raw": raw_input,
               "output_raw_list": [EXAMPLES[0]['output'], EXAMPLES[1]['output']],
               "token_probs": [[["The", 0.9], ["barn", 0.8]], [["Many", 0.77], ["people", 0.21]]],
               "beam_scores": [-0.021, -0.5]}
...
def get_raw_response(state_dict):
    ...
    res_raw = compute_answer(raw_input, generator_options, random_tickle, output_strings)
    res = res_raw.copy()
...
def api_compute(state_dict):
    return get_raw_response(state_dict)


def get_response(state):
    state_dict = state._state["data"]
    output = get_raw_response(state_dict)
...
if REST_API_PORT is not None and (not hasattr(st, 'already_started_server') or st.already_started_server != "Started"):
    ...
    # The following should be before app.run, but for some reason it causes things to hang atm
    st.write('''
