...
from macaw_utils import ALL_SLOTS_LC_DICT, GENERATOR_OPTIONS_DEFAULT, decompose_slots, get_raw_response, \
...
def render_output_slots(state, index):
    ...
    for slot, value in output_slots.items():
        if slot in ALL_SLOTS_LC_DICT:
            str_out += "**" + ALL_SLOTS_LC_DICT[slot] + ":** " + value + "  \n"
    st.write(str_out)
